...
class ActionModel:
    def __init__(self, initial_games=100, test_games=1000, goal_steps=2000, lr=1e-2, file='actionSave.npz'):
        ...
        self.training_data = np.load('../data/actionData.npy', allow_pickle=True)
    ...
    def initial_population(self):
        ...
        model = scoreModel.load('tfSave.npz')
        for i in range(self.initial_games):
            ...
            for _ in range(self.goal_steps):
                ...
                for action in range(-1, 2):
                    preds.append(scoreModel.forward_prop(model, np.append([action], prev_obs).reshape(5, -1))[-1])
            ...
            if i % 10 == 0:
                print(i)
        print('Finished generating observation -> action training data')
    ...
    def test(self, model):
        ...
        for _ in range(self.test_games):
            ...
            prev_obs = self.gen_obs(snake, food)
            for _ in range(self.goal_steps):
                preds = self.predict(prev_obs, model)
                action = np.argmax(np.array(preds))
                game_action = self.get_game_action(snake, action - 1)
                done, score, snake, food = game.step(game_action)
    ...
    def run(self):
        ...
        for _ in range(100):
            J, Z1, A1, Z2, A2 = self.forward_prop(self.X, self.y, model)
