...
class EGAARTD:
    def __init__(self, iterations=100, pop_size=100, mut_prob=0.7, elite_ratio=0.01, cross_prob=0.5, par_ratio=0.3, ar_N=1, td_N=1):
    ...
    def clip(self, x):
        ...
        tx = x[self.td_N * td_size:]
        tx[tx < self.clip_lo] = self.clip_lo
        tx[tx > self.clip_hi] = self.clip_hi
        x[self.td_N * td_size:] = tx
        return x
    ...
    def run(self):
        ...
        self.nn.test(model)
        np.savez('../saves/egaARTDSave.npz', W1=model['W1'], b1=model['b1'], W2=model['W2'], b2=model['b2'])
