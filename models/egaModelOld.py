...
class EGA:
    ...
    def decode(self, decoder):
        ...
        for i in range(1, len(self.ln)):
            W, decoder = np.split(decoder, [np.prod(self.ln[i])])
            ret['W' + str(i)] = W.reshape(self.ln[i])
            b, decoder = np.split(decoder, [self.ln[i][-1]])
            ret['b' + str(i)] = b
