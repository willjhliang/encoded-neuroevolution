...
class ScoreProb:
    def __init__(self, test_games=10, goal_steps=2000):
        self.test_games = test_games
        self.goal_steps = goal_steps
        self.helper = Helper(20, 20, 1)
    ...
    def test(self, model, games=None, gui=False):
        ...
        for g in range(games):
            ...
            prev_obs = self.helper.gen_obs(snake, food, 'Vec')
            for _ in range(self.goal_steps):
                ...
                if done:
                    ...
                    prev_obs = self.helper.update_obs(snake, food, 'Vec', prev_obs)
                    steps += 1
    ...
    def get_training_data(self):
        if os.path.isfile('data/scoreData.npz'):
            print('Loaded observation + action -> score data')
            return np.load('data/scoreData.npz', allow_pickle=True)
