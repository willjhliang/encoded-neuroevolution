from defNN import SnakeNN
from random import randint
import numpy as np
from collections import Counter


class GeneticAlgoEncode:
    ...
    def run(self):
        ...
        for t in range(1, self.iterations + 1):
            ...
            for p in range(self.pop_size):
                J = snakeNN.forward_prop(self.X, self.y, self.decode(pop[p]))[0]
                fitness[p] = J
            pop = pop[np.argsort(fitness)]
            fitness = fitness[np.argsort(fitness)]

            J = snakeNN.forward_prop(self.X, self.y, self.decode(pop[0]))[0]
            if t % 10 == 0:
        ...
        snakeNN.test_model(model)
        np.savez('gaEncodeModel.npz', W1=model['W1'], b1=model['b1'], W2=model['W2'], b2=model['b2'])


if __name__ == '__main__':
    GA = GeneticAlgoEncode()
    GA.run()
